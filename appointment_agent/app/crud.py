...
def register_new_patient(db: Session, patient_name: str, mobile_number: str, dob: str, gender: str,
                         email: Optional[str] = None):
    """Register a new patient in the system"""
    try:
        # Check if patient already exists
        existing_patient = get_patient_by_mobile(db, mobile_number)
        if existing_patient:
            return None, "Patient with this mobile number already exists"

        # Generate MRN (Medical Record Number)
        patient_count = db.query(models.Patient).count()
        patient_mrn = f"MRN{patient_count + 1:06d}"
        ...
        db.rollback()
        return None, f"Error registering patient: {str(e)}"
